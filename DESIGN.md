...
- **Built-in themes** ship with the app as starting points to copy and edit: Classic (the default dark look), Frutiger Aero, Aero Glass, Liquid Glass (clear glass over soft iridescent colour and glossy ribbons), Liquid Glass Dark (dark glass over black, with neon ribbons and curls of smoke), and Rainy Window (a blurred night city with rain falling outside, behind drops on the glass that refract the lights and slide down in fits and starts; the layers drift at different speeds as you scroll, and messages are glossy liquid glass bubbles).
- **Liquid glass is real refraction, like Apple's.** A theme can make its glass work as a lens: the background bends as it nears a pane's rounded edge, and splits into a faint rainbow there. It's done with an SVG displacement map used as a backdrop filter (`public/glass.js`), so it only works in Chromium browsers (Chrome, including on Android); elsewhere, and in Lite mode, the theme's ordinary blur is used. Decisions:
  ...
  - **Kept cheap**: a lens is built once per size and settings and shared by every element that matches (most bubbles are the same width), only the rim of its map is computed, big maps are drawn at half resolution, and nothing runs while scrolling.
  - **Two Chrome quirks are worked around**: Chrome shifts a lensed element's backdrop by how far its outer shadows reach (so glass.js shifts the lens back), and it doesn't show a lens transformed layers properly (so the Liquid Glass themes drift their layers with `background-position`, not `transform`).
...
- **Themes can offer sliders.** A theme declares options in its theme.json (a label, a range, and the CSS variable each sets), and Appearance shows them for the app theme and the open channel's theme. The values are saved server-wide, per theme. Rainy Window uses them for bubble transparency and blur, raindrop strength and parallax; the Liquid Glass themes for tint, refraction, rainbow edges and frost (and neon and smoke in the dark one).
