...
- **Liquid glass is real refraction, like Apple's.** A theme can make its glass work as a lens: the background bends as it nears a pane's rounded edge, and splits into a faint rainbow there. It's done with an SVG displacement map used as a backdrop filter (`public/glass.js`), so it only works in Chromium browsers (Chrome, including on Android); elsewhere, and in Lite mode, the theme's ordinary blur is used. Decisions:
  ...
  - **Two Chrome quirks are worked around**: Chrome shifts a lensed element's backdrop by how far its outer shadows reach (so glass.js shifts the lens back), and it doesn't show a lens transformed layers properly (so the Liquid Glass themes drift their layers with `background-position`, and Rainy Window moves its rain, drops and city with animated custom properties used in background and mask positions, never `transform`).
